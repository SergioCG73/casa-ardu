...
//4) Ultima produccion sin filtrar

$sqlSelect = "SELECT NumeroFabricacion
        FROM  fabricaciones_sin_filtrar
        WHERE producto_id = :producto
        ORDER BY NumeroFabricacion DESC
        LIMIT 1
        ";    
...
$stmt->bindParam(":producto", $producto, PDO::PARAM_STR);
$stmt->execute();
$ultimaSinFiltrar = $stmt->fetchColumn();
...
//4) Obtener los equipos del producto a fabricar disponibles
$sqlSelect = "SELECT Equipo_id, Estado, Tipo FROM equipos
             WHERE ProductoFabricado = :producto";
